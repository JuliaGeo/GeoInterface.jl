...
geomtrait(feature::Feature) = FeatureTrait()
geometry(f::Feature) = f.geometry 
properties(f::Feature) = f.properties
extent(f::Feature) = f.extent

"""
    FeatureCollection(features, [extent=nothing])
    FeatureCollection(; features, [extent=nothing])

A wrapper for multiple `Feature`s.

- `features`: an `AbstractVector` of an object that satisfies `isfeaature(obj)`.
- `extent`: an Extents.jl `Extent` object, or `nothing` if the extent is not known.
"""
struct FeatureCollection{T,A<:AbstractVector{T},B<:Union{Nothing,Extents.Extent}} <: AbstractVector{T}
    features::A
    extent::B
end
FeatureCollection(features) = FeatureCollection(features, nothing)
FeatureCollection(; features, extent=nothing) = FeatureCollection(features, nothing)

Base.parent(fc::FeatureCollection) = fc.features
Base.size(fc::FeatureCollection) = size(parent(fc))
Base.getindex(fc::FeatureCollection, I...) = getindex(parent(fc), I...)
Base.setindex!(fc::FeatureCollection, x, I...) = setindex!(parent(fc), x, I...)

isfeaturecollection(fc::Type{<:FeatureCollection}) = true
geomtrait(fc::FeatureCollection) = FeatureCollectionTrait()
nfeature(::FeatureCollectionTrait, fc::AbstractVector) = Base.length(fc)
getfeature(::FeatureCollectionTrait, fc::FeatureCollection) = parent(fc)
getfeature(::FeatureCollectionTrait, fc::FeatureCollection, i::Integer) = fc[i]
